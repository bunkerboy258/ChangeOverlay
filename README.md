...
快捷键设置采用按键捕获方式：点击输入框后直接按下想要的组合键，例如 Ctrl+Alt+O，框内会自动显示；按 Backspace 清除。如果组合键已被 Rider 中其他操作占用，会红字提示占用者并拒绝本次设置，保留原快捷键。修改后立即生效，快捷键保存在 Rider 当前键盘映射中，重启后仍然有效。
...
`启用或关闭更改覆盖` 只控制当前 Rider 会话，每次启动默认开启，不属于持久化设置。
